...
class PencilSketch:
    ...
    def __init__(self, width, height, bg_gray='resources/pencilsketch_bg.jpg'):
...
if __name__ == '__main__':
    ...
    img_rgb = cv2.imread("input2.png")
    img_rgb = cv2.resize(img_rgb, (1200,800))
    height, width, channels = img_rgb.shape
    cv2.imwrite("aaOriginal.jpg", img_rgb)

    try:
        Cartoonize = Cartoonizer()
        im = Cartoonize.render(img_rgb)
        im = cv2.resize(im, (750, 502))
        cv2.imwrite("aaCartoon15.jpg", im)
    except:
        cv2.imwrite("aaCartoon15.jpg", img_rgb)

    try:
        Cooling = CoolingFilter()
        im2 = Cooling.render(img_rgb)
        im2 = cv2.resize(im2, (750, 502))
        cv2.imwrite("aaCooling.jpg", im2)
    except:
        cv2.imwrite("aaCooling.jpg", img_rgb)

    try:
        Warming = WarmingFilter()
        im3 = Warming.render(img_rgb)
        im3 = cv2.resize(im3, (750, 502))
        cv2.imwrite("aaWarming.jpg", im3)
    except:
        cv2.imwrite("aaWarming.jpg", img_rgb)

    try:
        Pencil = PencilSketch(width, height)
        im4 = Pencil.render(img_rgb)
        im4 = cv2.resize(im4, (750, 502))
        cv2.imwrite("aaPencil.jpg", im4)
    except:
        cv2.imwrite("aaPencil.jpg", img_rgb)

    try:
        Gray = GrayScale()
        im5 = Gray.render(img_rgb)
        im5 = cv2.resize(im5, (750, 502))
        cv2.imwrite("aaGray.jpg", im5)
    except:
        cv2.imwrite("aaGray.jpg", img_rgb)
